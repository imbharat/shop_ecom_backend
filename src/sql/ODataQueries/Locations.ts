...
const SelectMin = `locations.location_id AS location_id,
...
const LocationODataExport = `SELECT ${SelectMin},
...
export const locationOData = (odata: OData) => {
  ...
  return `SELECT ${!!$select ? $select : SelectMin} ${LocationODataCommon} 
    ${!!$filter ? `WHERE ${$filter}` : ``}
    ${
      !!$orderby
        ? `ORDER BY ${$orderby}`
